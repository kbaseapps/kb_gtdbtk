...
def test_get_gtdbtk_params():
    ...
    assert p == ('5/6/7', 56, 0, 0, 10, 0, 0, 0)

    p = get_gtdbtk_params({'workspace_id': 92, 'input_object_ref': '104/67/3', 'min_perc_aa': 78.9, 'copy_proximals': 0, 'save_trees': 0})
    assert p == ('104/67/3', 92, 0, 0, 78.9, 0, 0, 0)


def test_get_gtdbtk_params_backwards_compatibility():
    p = get_gtdbtk_params({'workspace_id': 56, 'inputObjectRef': '8/9/10', 'copy_proximals': 0, 'save_trees': 0})
    assert p == ('8/9/10', 56, 0, 0, 10, 0, 0, 0)
